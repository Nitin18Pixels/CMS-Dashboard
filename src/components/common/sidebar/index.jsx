    ...
    <div className={`p-4 sidebar ${isOpen ? "d-block" : "d-none d-md-block"}`}>
      ...
      <ul className="list-unstyled">
        {MenuTypes.map((menu, index) => {
          ...
          return (
            <li key={menu.id} className={`p-2 ${isActive ? "active-menu" : ""}`}>
              {menu.submenu ? (
                <div
                ...
                <Link to={menu.link} className={`text-decoration-none ${isActive ? "active-link" : ""}`}>
                  {menu.icon}
                  <span className="ms-2">{isOpen && menu.menu}</span>
                </Link>
              )}
              {menu.submenu && (
                ...
                >
                  {menu.submenu.map((submenu) => {
                    ...
                    return (
                      <li key={submenu.submenuID} className={`p-2 ${isSubmenuActive ? "active-submenu" : ""}`}>
                        <Link to={submenu.link} className={`text-decoration-none ${isSubmenuActive ? "active-link" : ""}`}>
                          {submenu.label}
                        </Link>
                      </li>
                    );
